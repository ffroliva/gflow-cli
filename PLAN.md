...
- [x] **Prompt Expansion ("Creative Director" mode) — core shipped 2026-06-27 (PR #209 → develop).** `-e`/`--expand` on `gflow image t2i` and `video t2v` rewrites a terse prompt via Gemini's 5-component formula before submitting to Flow. `api/prompt_expander.py` (stdlib-urllib, never-fatal: missing key / 429-backoff / network / empty-clean all fall back to the original). `operations.expanded_prompt` column (migration 0008) records both original + submitted expansion (withheld under `history_prompts=redacted`). Reuses `GFLOW_CLI_GEMINI_API_KEY`; adds `GFLOW_CLI_GEMINI_MODEL` (default `gemini-2.5-flash`). MCP `expand` param mirrored per AGENTS.md §61. High-effort multi-agent review fixed 2 real bugs (empty-clean abort, --json stdout pollution) pre-merge. Memory `[[prompt-expansion-feature]]`.
  - **Phase B (Backlog — broaden surface):** wire `--expand` into the remaining generation commands — `image i2i`, `image t2i --prompts-file/--stdin` batch, `video i2v`/`r2v`, and `video chain`. Current impl is single-prompt `t2i`/`t2v` only (expansion happens in the Click body before the runner). The clean altitude is a request-build-layer hook (or `original_prompt` carried on the request DTO) so every surface inherits expansion without re-threading the kwarg through each recorder method. Watch the silent-misrecord hazard: a recorder call that records a prompt but forgets `original_prompt` persists the EXPANDED text as the user's original.
  - **Phase C (Backlog — quality):** consider lowering the per-attempt 30s timeout (worst-case ~120s blocking under sustained Gemini 429s before fallback); evaluate reusing `tenacity` (already a dep) instead of the hand-rolled backoff loop.
