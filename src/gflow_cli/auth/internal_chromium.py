...
async def _poll_session_until_authenticated(
    ...
    """Poll the NextAuth session endpoint until the Flow app sign-in completes.

    Non-AUTHENTICATED outcomes (including a transient VERIFICATION_ERROR) just
    mean "keep waiting" — the user may still be signing in. Raises
    ``AuthLoginTimeoutError`` on timeout and ``AuthBrowserRejectedError`` when
    Google routes to its rejection page.
    """
