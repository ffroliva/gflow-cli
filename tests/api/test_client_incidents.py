...
class TestDeferredParkOrdering:
    """#792: a migrated run's about:blank park must not outrun the bundle.

    The park sat in a bare ``finally`` inside the transport, so on the failure
    path it navigated away BEFORE ``_capture_incident`` read the page — whose own
    docstring promises to stage the bundle "while the page is still alive". Field
    bundles came back with ``tag_counts.div = 0``, a white screenshot and
    ``host_category = "other"`` beside a network journal proving the app was live.
    """

    async def test_the_incident_is_captured_before_the_deferred_park_runs(
        self, tmp_path: Path
    ) -> None:
        client = _client(tmp_path)
        seen: list[str] = []
        ...
        class FakePage:
            url = "https://flow.google.com/project/p1"

            async def goto(self, url: str, **_: Any) -> None:
                seen.append(f"park:{url}")
                type(self).url = url

        class FakeTransport:
            _deferred_park_pending = True

            async def park_deferred_page(self) -> None:
                type(self)._deferred_park_pending = False
        ...
        async def _capture(exc: BaseException, *, phase: str, route: str | None = None) -> None:
            seen.append(f"capture:{page.url}")

        client._capture_incident = _capture  # type: ignore[assignment]

        with pytest.raises(FlowAppError):
            await client._raise_with_incident(FlowAppError(detail="drift"), phase="video")

        assert seen == [
            "capture:https://flow.google.com/project/p1",
            "park:about:blank",
        ], seen

    async def test_a_magicmock_transport_never_trips_the_deferred_park(
        self, tmp_path: Path
    ) -> None:
        """`getattr(mock, ...)` is truthy for anything — the guard compares to
        True so a mocked transport in an unrelated test cannot be navigated."""
        from unittest.mock import MagicMock

        client = _client(tmp_path)
        client.transport = MagicMock()
        client._capture_incident = _noop_capture  # type: ignore[assignment]
        with pytest.raises(FlowAppError):
            await client._raise_with_incident(FlowAppError(detail="x"), phase="video")
        assert not client.transport.park_deferred_page.called


async def _noop_capture(exc: BaseException, *, phase: str, route: str | None = None) -> None:
    return None
