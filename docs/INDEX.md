...
| **[docs/TOOLS.md](TOOLS.md)** | Tools framework: the `gflow tools list/show/run` group, the `--tool/-t` option on generation commands, the TOML tool schema, how a tool is defined, the dormant My-Tools seam, MCP exposure + §61 parity | Working on `gflow_cli.tools`, adding a built-in tool, or wiring `--tool` into a command |
