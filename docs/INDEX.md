...
Slash commands for Claude Code, stored in `.claude/commands/gflow/`. All prefixed `/gflow:` to signal project scope.

| Command | Purpose | Call when… |
|---|---|---|
| `/gflow:check` | Auto-fix lint + format, report types and tests | Before every commit |
| `/gflow:plan` | Show current phase scope, sequence, and DoD | Starting a feature or unsure what's in scope |
| `/gflow:known-issues` | Surface open and mitigated issues | Before touching auth or reCAPTCHA |
