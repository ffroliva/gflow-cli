...
**Video commands:** `gflow video t2v`/`i2v` (and the video legs of `gflow run`) joined the policy in [#299](https://github.com/ffroliva/gflow-cli/issues/299) PR-A. The video pipeline only has a classic driver, so `auto` ≡ `classic` there: both verify the classic editor pre-submit and abort with exit 28 if it is unreachable. An env-sourced `agentic` **degrades to classic with a logged warning** (so a value set for image workflows can't hard-fail your video runs); only the explicit `--ui-mode agentic` flag errors, immediately at the CLI edge (exit 2), because no agentic video driver exists yet.
