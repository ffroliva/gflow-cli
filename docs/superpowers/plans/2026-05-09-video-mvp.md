...
| Path | Responsibility |
|---|---|
| `src/gflow_cli/api/video.py` | Pure value objects + body builders for video generation. No I/O. |
| `src/gflow_cli/api/recaptcha.py` | Site-key discovery + token minting via Playwright `page.evaluate`. |
| `src/gflow_cli/cli_video.py` | Click command group `video` with `t2v`, `i2v`, `batch`, `status` subcommands. |
| `src/gflow_cli/manifest.py` | TSV manifest parser for `video batch`. |
...
| Path | What changes |
|---|---|
| `src/gflow_cli/api/client.py` | Add `generate_video()` method using new `recaptcha` + `video` modules. |
| `src/gflow_cli/api/dto.py` | Add `VideoOperation` DTO + parser for the generate-video response. |
| `src/gflow_cli/cli.py` | Register the new `video` group from `cli_video`. |
| `src/gflow_cli/config.py` | Add `headless: bool = True` field. |
...
| Path | Why |
|---|---|
| `src/gflow_cli/providers/` (entire dir) | Superseded by `gflow_cli.api.client.FlowApiClient`. |
| `src/gflow_cli/models.py` | Domain DTOs folded into `gflow_cli/api/dto.py` already. |
...
**Files:**
- Create: `src/gflow_cli/api/video.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'gflow_cli.api.video'`.

- [ ] **Step 1.3: Implement `src/gflow_cli/api/video.py`**
...
**Files:**
- Create: `src/gflow_cli/api/recaptcha.py`
...
```bash
git add src/gflow_cli/api/recaptcha.py tests/api/test_recaptcha.py
...
**Files:**
- Modify: `src/gflow_cli/api/recaptcha.py`
...
Append to `src/gflow_cli/api/recaptcha.py`:
...
- Empty/non-string token -> RecaptchaError with a remediation hint
  pointing at GFLOW_CLI_HEADLESS=false (the most common fix when
...
Append to `src/gflow_cli/api/dto.py`:
...
class TestGenerateVideo:
    async def test_t2v_posts_expected_body(self, client: FlowApiClient) -> None:
        ...
        with (
            ...
            patch("gflow_cli.api.client.TokenMinter") as MinterCls,
        ):
...
In `src/gflow_cli/api/client.py`, add imports near the top (after the existing imports):
...
- [ ] **Step 4.6: Update `gflow_cli.api.__init__` exports**

Edit `src/gflow_cli/api/__init__.py`:
...
```bash
git add src/gflow_cli/api/ tests/api/test_client_generate_video.py
...
**Files:**
- Create: `src/gflow_cli/manifest.py`
...
- [ ] **Step 5.3: Implement `src/gflow_cli/manifest.py`**
...
```bash
git add src/gflow_cli/manifest.py tests/test_manifest.py
...
## Task 6: `GFLOW_CLI_HEADLESS` setting + plumb through `FlowApiClient`
...
In `src/gflow_cli/config.py`, inside the `Settings` class (under `# --- runtime`):
...
```bash
git add src/gflow_cli/config.py tests/test_config.py
git commit -m "feat(config): add GFLOW_CLI_HEADLESS setting (default True)
...
**Files:**
- Create: `src/gflow_cli/cli_video.py`
...
Expected: FAIL — `ModuleNotFoundError: No module named 'gflow_cli.cli_video'`.

- [ ] **Step 7.3: Implement `src/gflow_cli/cli_video.py`**
...
In `src/gflow_cli/cli.py`, after the `main` group definition but before the existing commands, add:
...
```bash
git add src/gflow_cli/cli_video.py src/gflow_cli/cli.py tests/test_cli_video.py
...
- Headless mode honoured from GFLOW_CLI_HEADLESS.
...
Append to `src/gflow_cli/cli_video.py`:
...
```bash
git add src/gflow_cli/cli_video.py tests/test_cli_video.py
...
def batch(
    ...
    from gflow_cli.manifest import parse_manifest
...
**Files:**
- Delete: `src/gflow_cli/providers/` (whole directory)
- Delete: `src/gflow_cli/models.py`
- Delete: `tests/providers/` (whole directory)
- Delete: `tests/test_models.py`
- Modify: `src/gflow_cli/cli.py` (remove the now-orphaned legacy `_make_provider` import)

- [ ] **Step 10.1: Audit imports**

```bash
cd C:/development/github/gflow-cli
uv run python -c "import gflow_cli.providers.flow; import gflow_cli.models" 2>&1 | tail -5
```
...
import subprocess
out = subprocess.run(
    ['grep','-rn','-E','gflow_cli\\.providers|gflow_cli\\.models','src','tests','--include=*.py'],
...
Expected output (the only callers): `src/gflow_cli/cli.py` (one import + `_make_provider` helper).

- [ ] **Step 10.3: Remove the legacy helper from cli.py**

Open `src/gflow_cli/cli.py` and delete:

1. The import `from gflow_cli.providers.flow import FlowProvider`
...
```bash
rm -r src/gflow_cli/providers
rm src/gflow_cli/models.py
...
The gflow_cli.providers.FlowProvider stub and gflow_cli.models domain
DTOs were placeholders before the api/ package landed. Their
responsibilities are now covered by:
- gflow_cli.api.client.FlowApiClient (was providers.flow.FlowProvider)
- gflow_cli.api.dto (was models)
- gflow_cli.api.video (new — composes models + body builders)
...
Edit `src/gflow_cli/__init__.py`:
...
```python
    assert gflow_cli.__version__ == "0.1.0"
```

to:

```python
    assert gflow_cli.__version__ == "0.2.0a1"
```
...
```bash
git add pyproject.toml src/gflow_cli/__init__.py tests/test_smoke.py
...
- pydantic-settings config (GFLOW_CLI_*) + XDG-aware paths
...
- [ ] Coverage ≥ 80% on `src/gflow_cli/api/`
