...
**Status:** Draft v1 (brainstorm approved by Flavio on 2026-05-14)
...
```python
parse_prompt_lines(text: str, *, source_label: str) -> tuple[str, ...]
```
...
- return retained prompts in source order
...
2. `gflow image t2i "p1" "p2" "p3" --aspect 9:16 --model nano2` runs all three
   prompts in one `FlowApiClient` session and one Flow project.
3. Multi-prompt positional mode passes the same global `aspect`, `model`, and
   `count` to every prompt item.
4. `--prompts-file p.txt` with five lines containing three prompt lines, one
   blank line, and one whole-line `#` comment produces exactly three prompt
   items in source order.
5. `--stdin` uses the same parser and execution path as `--prompts-file`.
6. Passing more than one prompt source, for example positional prompt(s) plus
   `--prompts-file`, fails with a clear usage error before profile resolution
   or browser/API work.
7. Passing `--seed` in multi-prompt mode fails with a clear usage error before
   profile resolution or browser/API work.
8. Multi-prompt mode rejects zero parsed prompts and more than 50 parsed prompts
   with clear errors.
9. Multi-prompt mode writes outputs using `prompt_<prompt-index>_<variation-index>.png`.
10. Multi-prompt mode supports `--continue-on-error` and `--fail-fast` with the
    same final exit-code semantics as `gflow run`.
11. The implementation preserves structured per-prompt outcomes containing at
    least prompt index, prompt text, status, output paths, and exit code.
12. Docs in `docs/USAGE.md`, `README.md`, and `CHANGELOG.md` describe the new
    input surfaces, file format, error semantics, seed limitation, and output
    naming.
13. Tests cover the required BDD scenarios from the kickoff prompt.
