...
**v0.9.1 — alpha.** Image (T2I / I2I / upload) + Video T2V / I2V / R2V live end-to-end on the `ui_automation` transport against live Pro/Ultra accounts, with a video `--model` picker (5 Veo models) and `--duration` / `--count`. New in v0.9.1: locale-invariant Flow selectors for non-English Chrome/Flow sessions, fixed I2V start/end-frame attachment, cross-profile media lookup fixes, first-run catalog migration fixes, and Windows-safe `gflow auth list` output. Only video `batch` (manifest runner) is still queued for Phase B — use a shell for-loop until then ([USAGE](USAGE.md#gflow-video-batch)). Three earlier HTTP transport strategies live under `src/gflow_cli/api/transports/experimental/` (`evaluate_fetch` / `bearer` / `sapisidhash`); the production path is `ui_automation`.

**Develop (unreleased, post-v0.9.1):** *(empty — develop is the staging branch for v0.10 work).*
