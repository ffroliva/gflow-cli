...
| `36` | `FlowHostMigratedError` | Flow served the project from `flow.google.com` — the origin Google is migrating the app onto — whose frontend renders none of the controls gflow drives. Not selector drift (23) | **Not retryable.** The handoff is a per-account setting the labs.google app applies on every load — measured 5/5 and 7/7 with no flap — so once the account is flagged, re-running will not land the old frontend. The REST surface (`gflow project list`, `gflow data …`) still works. See [#639](https://github.com/ffroliva/gflow-cli/issues/639) |
