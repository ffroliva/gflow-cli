...
| `37` | `InsufficientCreditsError` | The account has no Flow credits left for this model, so Flow **replaced** the submit control with its `Insufficient credits warning` instead of disabling it. Measured 2026-09-07 by A/B on a drained vs a funded account. Explicitly **not** selector drift (23): reporting it as drift told users to file a frontend bug for an empty wallet | Check the balance with `gflow credits user`, then top up or wait for the allowance to reset. Nothing was submitted, so no credit was spent. `gflow image` draws on a separate daily quota and may still work |
