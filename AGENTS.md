...
- Errors as RFC 9457 Problem Details with stable per-class exit codes (3–30, e.g. 11 is `ConfigurationError` — including `ProfileLockedError` for same-profile lease contention, 16 is the `DataStoreError` family, 19 `SceneConcatError`, 20 `FrameExtractionError`, 21 `ChainPartialError`, 22 `UpscaleUnavailableError`, 25 `FlowAgentUiError`, 28 `UiModeUnavailableError`, 29 `MentionIndexUnavailableError`, 30 `QueueSchemaError`). See `src/gflow_cli/errors.py::EXIT_CODE_MAP` for the complete mapping.
...
- **Locale-Invariance Discipline for UI Automation**: **Never** write single-language or English-only Playwright UI selectors for DOM elements, overlays, announcements, menus, tabs, or buttons. All DOM selectors in `src/gflow_cli/api/transports/` must adhere to the two-tier cascade pattern established in Issue #24: **Tier 1 (Structural / ARIA / Hyperlink / Icon anchors)** — e.g. `a[href*='changelog']`, `button:has(i.google-symbols:text('close'))`, `div[type='button'][aria-haspopup='dialog']` — which are language-invariant; followed by **Tier 2 (Multi-locale text cascades)** spanning supported locales (EN, PT, ES, DE, FR, IT, JA, ZH, KO, etc.). Single-language `has-text(...)` additions without structural anchors or multi-locale cascades are strictly forbidden.
