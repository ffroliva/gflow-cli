...
- **The media picker's library view is now aligned to the target project before any UUID asset lookup (#287 live repro — primary suspected mechanic).** `gflow video i2v <media-uuid> --project <id>` raised `TransportTimeoutError` ("Start frame asset ... could not be located in the media picker") on a crowded project for an asset that WAS in the project and in the local catalog, while the same command worked from a small scratch project. Primary hypothesis (project owner): the picker's library component has its OWN project selection — its search/grid is per-project, and `--project` only navigates the EDITOR, so the picker can open on a *different* active project (plausibly the most recently created one), making the target asset unreachable no matter how deep the grid is scrolled. New `_sync_picker_project` runs each time the frame-slot / Add-Media dialog opens: it derives the target project from the editor URL, probes the picker for a project-selector trigger (combobox / listbox-or-menu-haspopup cascade — hypothesis-driven, to be refined from live DOM dumps), no-ops when the trigger already references the target project id or when no selector exists (older cohort), and otherwise opens the dropdown and clicks the first option referencing the project id — a miss closes the dropdown (Escape) and proceeds, leaving the asset lookup as the authority.
...
- **Every picker-lookup decision point now emits structured telemetry, and a final not-found leaves a bounded DOM dump (#287 diagnosis).** The first live verification failed with zero events from the new code paths, making the failing layer indistinguishable (search tiers vs progress probe vs tile matcher vs wrong library project). New `ui_automation_video.*` events: `picker_project_selector_absent` / `picker_project_already_active` / `picker_project_switched` / `picker_project_switch_miss` / `picker_project_sync_skipped` (project alignment), `picker_search_tier` (term, found, rendered-tile count) and `picker_search_unavailable`, `picker_scroll_probe` (per scroll: rendered-tile count + new-tile delta) and `picker_scroll_done` (termination reason: found / stall / legacy_budget / ceiling, total attempts), and `existing_asset_not_found` (media id, project id, screenshot + dump paths). On a final not-found, `_capture_picker_dom_dump` writes `debug_picker_dom_<uuid8>.json` to the out-dir — tile count, the first 3 tiles' outerHTML truncated to 500 chars (enough to see which attribute carries the media identity in a given cohort), the dialog's aria/role/data attributes, the project-selector candidates' outerHTML, and whether the target project id appears in the dialog at all — plus a `debug_picker_miss_<uuid8>.png` screenshot; both follow the 0.32.1 None-on-capture-failure contract (never report a file that was not written) (#287).
